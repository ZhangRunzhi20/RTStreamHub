...
typedef struct {            /* tcp control type */
    ...
    SSL *ssl;               //TLS connect
    SSL_CTX *ctx;           //TLS add by zrz 2025.2.4
    int ssl_connect;        //TLS connect flag 1: wait, 2:sucess
} tcp_t;
                          ...
                          char *passwd, char *mntpnt, char *str)
{
    ...
    if ((p=strchr(p,'/'))) {
        if ((q=strchr(p+1,':'))) {//Determine if there is a mount point
...
static int ssl_recv_nb(SSL *ssl, uint8_t *buff, int n)
{
    struct timeval tv = {0};  // 立即返回
    fd_set rs;
    int ret, nr, sock;
    ...
    // 先用 select() 检查 socket 是否有数据可读
    FD_ZERO(&rs);
    FD_SET(sock, &rs);
    ret = select(sock + 1, &rs, NULL, NULL, &tv);
    if (ret <= 0) return ret; // 如果不可读，直接返回

    // 调用 SSL_read() 读取数据
    nr = SSL_read(ssl, buff, n);
    if (nr <= 0) {
...
static int ssl_send_nb(SSL *ssl, uint8_t *buff, int n)
{
    struct timeval tv = {0};
    fd_set ws;
    int ret, ns, sock;
    ...
    // 使用 select 检查 socket 是否可写
    FD_ZERO(&ws);
    FD_SET(sock, &ws);
    ret = select(sock + 1, NULL, &ws, NULL, &tv);
    if (ret <= 0) return ret; // 如果不可写，直接返回

    // 进行 SSL 发送
    ns = SSL_write(ssl, buff, n);
    if (ns <= 0) {
...
static void discontcp(tcp_t *tcp, int tcon)
{
    ...
    closesocket(tcp->sock);
    trace(1,"close tcp client socket: %d\n",tcp->sock);
    tcp->state=0;
    tcp->tcon=tcon;
    tcp->tdis=tickget();
}
...
static int accsock(tcpsvr_t *tcpsvr, char *msg)
{
    ...
    tracet(1,"accsock: connected sock=%d addr=%s i=%d\n",
...
{/* debug by xiaogongwei 20220111*/
    int i,nr,err;
    unsigned int current_tick = 0, diff_tick = 0;
    int cut_time = CUT_TCP_SERVER*10*1000;
    ...
    for (i=0;i<MAXCLI;i++) {
        ...
        if ((nr=recv_nb(tcpsvr->cli[i].sock,buff,n))==-1) {
            ...
            discontcp(&tcpsvr->cli[i],ticonnect);
            trace(1,"readtcpsvr 1\n");
            updatetcpsvr(tcpsvr,msg);
        }
        if (nr>0) {
            /*
            char tempbuff[512] = {0};
            memcpy(tempbuff, buff, nr);
    ...
    /* if long time no data close by xiaogongwei*/
    for (i=0;i<MAXCLI;i++) {
        // current
        current_tick = tickget();
        if(tcpsvr->cli[i].tact == 0) continue;
        diff_tick = current_tick - tcpsvr->cli[i].tact;
        if(tcpsvr->cli[i].state!=0&&abs(diff_tick) > cut_time){
            discontcp(&tcpsvr->cli[i],ticonnect);
            trace(1,"readtcpsvr 2\n");
            updatetcpsvr(tcpsvr,msg);
        }
    }
...
static int consock_ssl(tcpcli_t *tcpcli, char *msg)
{
    int stat,err;

    tracet(4,"consock_ssl: sock=%d\n",tcpcli->svr.sock);
    ...
    if ((stat=connect_nb(tcpcli->svr.sock,(struct sockaddr *)&tcpcli->svr.addr,
        ...
        tracet(2,"consock_ssl: connect error sock=%d err=%d\n",tcpcli->svr.sock,err);
    ...
    tcpcli->svr.ssl_connect = 0;
    tcpcli->svr.ssl = SSL_new(tcpcli->svr.ctx);
    if (!tcpcli->svr.ssl) {
        tracet(2,"consock_ssl: create ssl fail,close socket: %d\n",tcpcli->svr.sock);
        discontcp(&tcpcli->svr,tcpcli->tirecon);
    }


    if (SSL_set_fd(tcpcli->svr.ssl,tcpcli->svr.sock) != 1) {
        char err_msg[256];
        unsigned long err = ERR_get_error();
        ERR_error_string_n(err,err_msg,sizeof(err_msg));
        fprintf(stderr, "SSL_set_fd failed: %s\n", err_msg);
        tracet(2,"consock_ssl: SSL_set_fd fail,socket: %d, %s\n",tcpcli->svr.sock,err_msg);
        SSL_free(tcpcli->svr.ssl);
        tcpcli->svr.ssl = NULL;
        discontcp(&tcpcli->svr,tcpcli->tirecon);
    }
    tcpcli->svr.ssl_connect = 1; //wait
    sprintf(msg,"%s",tcpcli->svr.saddr);
    tracet(3,"consock_ssl: connected sock=%d addr=%s\n",tcpcli->svr.sock,tcpcli->svr.saddr);
    tcpcli->svr.state=2;
...
/* open tcp client over ssl -----------------------------------------------------------*/
static tcpcli_t *opentcpcli_ssl(const char *path, char *msg)
{
    ...
    *tcpcli=tcpcli0;
    ...
    if (!tcpcli->svr.ctx) {
        trace(1,"TLS context initialization failed");
        exit(0);
    }
...
/* wait socket connect over ssl -------------------------------------------------------*/
static int waittcpcli_ssl(tcpcli_t *tcpcli, char *msg)
{
    int ret;
    tracet(4,"waittcpcli_ssl: sock=%d state=%d\n",tcpcli->svr.sock,tcpcli->svr.state);
    ...
    if (tcpcli->svr.state==1) { /* wait */
        if (!consock_ssl(tcpcli,msg)) return 0;
    }
    if (tcpcli->svr.state==2 && tcpcli->svr.ssl_connect==1) {
        ret=SSL_connect(tcpcli->svr.ssl);
        if (ret !=1) {
            int err = SSL_get_error(tcpcli->svr.ssl,ret);
            if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                tracet(2,"SSL_connect connecting, socket: %d\n",tcpcli->svr.sock);
                return 0;
            }
            else {
                char err_msg[256];
                ...
                tracet(2,"waittcpcli_ssl: SSL_connect fail,socket: %d, %s\n",tcpcli->svr.sock,err_msg);
                SSL_free(tcpcli->svr.ssl);
                tcpcli->svr.ssl = NULL;
                discontcp(&tcpcli->svr,tcpcli->tirecon);
            }
            ...
            tcpcli->svr.ssl_connect=2;
            tracet(2,"SSL_connect sucess, socket: %d\n",tcpcli->svr.sock);
        }
    }
    if (tcpcli->svr.state==2 && tcpcli->svr.ssl_connect==2) { /* connect */
        if (tcpcli->toinact>0&&
            (int)(tickget()-tcpcli->svr.tact)>tcpcli->toinact) {
            sprintf(msg,"timeout");
            tracet(2,"waittcpcli_ssl: inactive timeout sock=%d\n",tcpcli->svr.sock);
            SSL_free(tcpcli->svr.ssl);
            tcpcli->svr.ssl = NULL;
            discontcp(&tcpcli->svr,tcpcli->tirecon);
            return 0;
...
static int waittcpcli(tcpcli_t *tcpcli, char *msg)
{
    ...
    if (tcpcli->svr.state==2) { /* connect */
        if (tcpcli->toinact>0&&
            (int)(tickget()-tcpcli->svr.tact)>tcpcli->toinact) {
            sprintf(msg,"timeout");
            tracet(1,"waittcpcli: inactive timeout sock=%d\n",tcpcli->svr.sock);
            discontcp(&tcpcli->svr,tcpcli->tirecon);
            return 0;
        }
    }
...
/* read tcp client -----------------------------------------------------------*/
static int readtcpcli_ssl(tcpcli_t *tcpcli, uint8_t *buff, int n, char *msg)
{
    int nr,err;

    tracet(4,"readtcpcli_ssl: sock=%d\n",tcpcli->svr.sock);

    if (!waittcpcli_ssl(tcpcli,msg)) return 0;

    if ((nr=ssl_recv_nb(tcpcli->svr.ssl,buff,n))==-1) {
        if ((err=errsock())) {
            tracet(2,"readtcpcli_ssl: recv error sock=%d err=%d\n",tcpcli->svr.sock,err);
            sprintf(msg,"recv error (%d)",err);
        }
        else {
            sprintf(msg,"disconnected");
        }
        tracet(2,"readtcpcli_ssl: ssl_recv_nb fail,socket: %d\n",tcpcli->svr.sock);
        SSL_free(tcpcli->svr.ssl);
        tcpcli->svr.ssl = NULL;
        discontcp(&tcpcli->svr,tcpcli->tirecon);
        return 0;
...
/* write tcp client ----------------------------------------------------------*/
static int writetcpcli_ssl(tcpcli_t *tcpcli, uint8_t *buff, int n, char *msg)
{
    int ns,err;

    tracet(3,"writetcpcli_ssl: sock=%d state=%d n=%d\n",tcpcli->svr.sock,tcpcli->svr.state,n);

    if (!waittcpcli_ssl(tcpcli,msg)) return 0;
    if ((ns=ssl_send_nb(tcpcli->svr.ssl,buff,n))==-1) {
        if ((err=errsock())) {
            tracet(2,"writetcpcli_ssl: send error sock=%d err=%d\n",tcpcli->svr.sock,err);
            sprintf(msg,"send error (%d)",err);
        }
        tracet(2,"writetcpcli_ssl: ssl_send_nb fail,socket: %d\n",tcpcli->svr.sock);
        SSL_free(tcpcli->svr.ssl);
        tcpcli->svr.ssl = NULL;
        discontcp(&tcpcli->svr,tcpcli->tirecon);
        return 0;
    ...
    tracet(5,"writetcpcli_ssl: exit sock=%d ns=%d\n",tcpcli->svr.sock,ns);
    return ns;
}
...
static int reqntrip_c_ssl(ntrip_t *ntrip, char *msg)
{
    char buff[MAXSTRPATH+1024],user[514],*p=buff;

    tracet(3,"reqntrip_c_ssl: state=%d\n",ntrip->state);
    ...
    if (writetcpcli_ssl(ntrip->tcp,(uint8_t *)buff,p-buff,msg)!=p-buff) return 0;

    tracet(3,"reqntrip_c_ssl: send request state=%d ns=%d\n",ntrip->state,p-buff);
    tracet(5,"reqntrip_c_ssl: n=%d buff=\n%s\n",p-buff,buff);
    ntrip->state=1;
    return 1;
}
...
static int rspntrip_c_ssl(ntrip_t *ntrip, char *msg)
{
    int i;
    char *p,*q;

    tracet(3,"rspntrip_c_ssl: state=%d nb=%d\n",ntrip->state,ntrip->nb);
    ntrip->buff[ntrip->nb]='0';
    tracet(5,"rspntrip_c_ssl: n=%d buff=\n%s\n",ntrip->nb,ntrip->buff);
    ...
    if ((p=strstr((char *)ntrip->buff,NTRIP_RSP_OK_CLI))) { /* ok */
        ...
        ntrip->state=2;
        sprintf(msg,"%s/%s",ntrip->tcp->svr.saddr,ntrip->mntpnt);
        tracet(3,"rspntrip_c_ssl: response ok nb=%d\n",ntrip->nb);
        return 1;
    }
    if ((p=strstr((char *)ntrip->buff,NTRIP_RSP_SRCTBL))) { /* source table */
        if (!*ntrip->mntpnt) { /* source table request */
            ...
            tracet(3,"rspntrip_c_ssl: receive source table nb=%d\n",ntrip->nb);
            return 1;
        }
        sprintf(msg,"no mountp. reconnect...");
        tracet(2,"rspntrip_c_ssl: no mount point nb=%d\n",ntrip->nb);
        ntrip->nb=0;
        ntrip->buff[0]='\0';
        ntrip->state=0;
        SSL_free(ntrip->tcp->svr.ssl);
        ntrip->tcp->svr.ssl = NULL;
        discontcp(&ntrip->tcp->svr,ntrip->tcp->tirecon);
    }
    ...
    tracet(5,"rspntrip_c_ssl: exit state=%d nb=%d\n",ntrip->state,ntrip->nb);
    return 0;
}
...
static int rspntrip_c(ntrip_t *ntrip, char *msg)
{
    ...
    if ((p=strstr((char *)ntrip->buff,NTRIP_RSP_SRCTBL))) { /* source table */
        ...
        ntrip->state=0;
        discontcp(&ntrip->tcp->svr,ntrip->tcp->tirecon);
        trace(1,"rspntrip_c 3\n");
    }
    tracet(5,"rspntrip_c: exit state=%d nb=%d\n",ntrip->state,ntrip->nb);
    return 0;
}

/* wait ntrip request/response over ssl -----------------------------------------------*/
static int waitntrip_ssl(ntrip_t *ntrip, char *msg)
{
    int n;
    char *p;

    tracet(4,"waitntrip_ssl: state=%d nb=%d\n",ntrip->state,ntrip->nb);

    if (ntrip->state<0) return 0; /* error */
    if (ntrip->tcp->svr.state<2|| ntrip->tcp->svr.ssl_connect<2) ntrip->state=0; /* tcp disconnected */

    if (ntrip->state==0) { /* send request */
        if (!(ntrip->type==0?reqntrip_s(ntrip,msg):reqntrip_c_ssl(ntrip,msg))) {
            return 0;
        }
        tracet(3,"waitntrip_ssl: state=%d nb=%d\n",ntrip->state,ntrip->nb);
    }
    if (ntrip->state==1) { /* read response */
        p=(char *)ntrip->buff+ntrip->nb;
        if ((n=readtcpcli_ssl(ntrip->tcp,(uint8_t *)p,NTRIP_MAXRSP-ntrip->nb-1,msg))==0) {
            tracet(5,"waitntrip_ssl: readtcp n=%d\n",n);
            return 0;
        }
        ntrip->nb+=n; ntrip->buff[ntrip->nb]='\0';

        /* wait response */
        return ntrip->type==0?rspntrip_s(ntrip,msg):rspntrip_c_ssl(ntrip,msg);
    }
    return 1;
}
...
static ntrip_t *openntrip(const char *path, int type, char *msg)
{
    ...
    if (ntrip->sslstatus==1) {
        /* open tcp client stream */
        if (!(ntrip->tcp=opentcpcli_ssl(tpath,msg))) {
...
/* close ntrip ---------------------------------------------------------------*/
static void closentrip_ssl(ntrip_t *ntrip)
{
    tracet(3,"closentrip: state=%d\n",ntrip->state);

    SSL_shutdown(ntrip->tcp->svr.ssl);
    SSL_free(ntrip->tcp->svr.ssl);
    closetcpcli(ntrip->tcp);
    SSL_CTX_free(ntrip->tcp->svr.ctx);
    free(ntrip);
}
...
/* read ntrip ----------------------------------------------------------------*/
static int readntrip_ssl(ntrip_t *ntrip, uint8_t *buff, int n, char *msg)
{
    int nb;

    tracet(4,"readntrip:\n");

    if (!waitntrip_ssl(ntrip,msg)) return 0;
    ...
    return readtcpcli_ssl(ntrip->tcp,buff,n,msg);
}
...
static void wait_ntripc(ntripc_t *ntripc, char *msg)
{
    ...
    //change by zhangrunzhi
    unsigned int current_tick = 0, diff_tick = 0;
    int cut_time = CUT_TCP_SERVER*1000;
    for (i=0;i<MAXCLI;i++) {
        // current
        current_tick = tickget();
        if(ntripc->tcp->cli[i].tact == 0) continue;
        diff_tick = current_tick - ntripc->tcp->cli[i].tact;
        if(ntripc->tcp->cli[i].state!=0&&abs(diff_tick) > cut_time){
            discon_ntripc(ntripc,i);
            trace(1,"client %d time out\n",i);
            updatetcpsvr(ntripc->tcp,msg);
        }
    ...
    for (i=0;i<MAXCLI;i++) {
        ...
        trace(1,"tcp server sock: %d,tcp client%d already,sock : %d\n",ntripc->tcp->svr.sock,i,ntripc->tcp->cli[i].sock);
        if ((n=recv_nb(ntripc->tcp->cli[i].sock,buff,nmax))==-1) {
            if ((err=errsock())) {
                tracet(2,"wait_ntripc: recv error sock=%d err=%d client%d\n",
                       ntripc->tcp->cli[i].sock,err,i);
            }
            discon_ntripc(ntripc,i);
            continue;
        }
...
static int writentripc(ntripc_t *ntripc, uint8_t *buff, int n, char *msg)
{
    ...
    for (i=0;i<MAXCLI;i++) {
        ...
        if (ns<n) {
            ...
            trace(1,"tcp server socket is %d, tcp client%d sock is %d, client%d is processing\n",ntripc->tcp->svr.sock,i,ntripc->tcp->cli[i].sock,i);
            ntripc->tcp->cli[i].tact=tickget();
        }
    }
...
extern void strclose(stream_t *stream)
{
    ...
    if (stream->port) {
        switch (stream->type) {
            ...
            case STR_NTRIPCLI: {
                if (((ntrip_t  *)stream->port)->sslstatus==1) {
                    closentrip_ssl ((ntrip_t  *)stream->port); break;
                }
                else {
                    closentrip ((ntrip_t  *)stream->port); break;
                }
            }
...
extern int strread(stream_t *stream, uint8_t *buff, int n)
{
    ...
    switch (stream->type) {
        ...
        case STR_NTRIPCLI: {
            if (((ntrip_t  *)stream->port)->sslstatus ==1 ) {
                nr=readntrip_ssl ((ntrip_t  *)stream->port,buff,n,msg); break;
            }
            else {
                nr=readntrip ((ntrip_t  *)stream->port,buff,n,msg); break;
            }
        }
